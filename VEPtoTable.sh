...
{
VCF=$1
FILENAME=`echo $VCF | cut -f 1 -d .`
#VEP=$VCF.VEP
VEP=`echo $VCF | sed 's/.VEP.vcf/.VEP/g'`
...
echo $otherinfo $IMPACT $SYMBOL $BIOTYPE $GMAF $SIFT $PolyPhen $GT $DP $cobertura_TECIDO $cobertura_SANGUE
...
do
	...
	cat ~murilo/Dropbox/sh_scripts/VEPtoTable.header $vcf.txt > $vcf.txt2 &&
	mv  $vcf.txt2 $vcf.xls &&
        rm $vcf.txt &&
